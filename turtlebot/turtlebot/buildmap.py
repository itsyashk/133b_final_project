...
class CustomNode(Node):
    ...
    def bresenham(self, start, end):
        ...
        # If there's no difference, just return empty
        if dx == 0 and dy == 0:
            return []

        # If |dx| >= |dy|, step along x
        if abs(dx) >= abs(dy):
            # If dx == 0, step would be 0 => crash. So handle that case:
            if dx == 0:
                return []
            step = 1 if dx > 0 else -1
            line = []
            for u in range(xs, xe, step):
                # Use float(dx) to avoid integer division in Python 3
                v = int(ys + (ye - ys) / float(dx) * (u + 0.5 - xs))
                line.append((u, v))
            return line
        else:
            # If dy == 0, step would be 0 => crash. So handle that case:
            if dy == 0:
                return []
            step = 1 if dy > 0 else -1
            line = []
            for v in range(ys, ye, step):
                # Use float(dy) to avoid integer division in Python 3
    ...
    def laserCB(self, msg):
        ...
        if self.last_u is not None and self.last_v is not None:
            path_cells = self.bresenham((self.last_u, self.last_v), (u, v))
            for (a, b) in path_cells:
                if 0 <= a < WIDTH and 0 <= b < HEIGHT:
                    self.logoddsratio[b, a] += 1.0
